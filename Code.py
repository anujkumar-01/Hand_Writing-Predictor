...
Page 9
     # Separate the data into feature data and target
data (X and y, respectively)
X = data[feature_cols]
y = data[target_col]
 # Apply PCA by fitting the data with only 60
dimensions
...
3, test_size=0.4, random_state=42)
...
y_pred = np.empty(len(y_test), dtype=np.int)
 start = time()
        ...
        # Find the nearest neighbors indices for each
sample in the test set
 kneighbors = knn_clf.kneighbors(X_test,
